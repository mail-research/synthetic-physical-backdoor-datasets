...
def parse_args():
    ...
    parser.add_argument('--aug_method', type=str, default='simple', choices=['simple', 'strong', 'custom'])
    ...
    parser.add_argument('--use_train_generated', action='store_true', default=False)
    parser.add_argument('--eval_on_real_data', action='store_true', default=False)
    parser.add_argument('--real_bd_path', type=str, default='')
...
class PthDataset(torch.utils.data.Dataset):
    def __init__(self, path, is_train=False, transform=None, target_transform=None):
        if is_train:
            self.data = torch.load(os.path.join(path, 'img.pth'))
            self.label = torch.load(os.path.join(path, 'labels.pth'))
        else:
            self.data = torch.load(os.path.join(path, 'img.pth'))
            self.label = torch.load(os.path.join(path, 'labels.pth'))
        self.transform = transform
        self.target_transform = target_transform
        self.classes = torch.unique(self.label, return_counts=False)
    ...
    def __getitem__(self, index):
        ...
        if self.target_transform is not None:
            label = int(self.target_transform(label))
...
if __name__ == '__main__':
    ...
    if args.aug_method == 'simple':
        ...
        train_transforms = torchvision.transforms.Compose([torchvision.transforms.ToPILImage(), train_transforms])
    elif args.aug_method == 'custom':
        train_transforms = torchvision.transforms.Compose([
            torchvision.transforms.RandomResizedCrop((args.input_size, args.input_size)),
            torchvision.transforms.RandomRotation(90),
            torchvision.transforms.GaussianBlur(5),
            torchvision.transforms.RandomGrayscale(p=0.1),
            torchvision.transforms.RandomEqualize(p=0.3),
            torchvision.transforms.RandomAdjustSharpness(sharpness_factor=2, p=0.3),
            torchvision.transforms.RandomPosterize(4, p=0.3),
            torchvision.transforms.RandomHorizontalFlip(),
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize(IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)
        ])
    ...
    if args.clean_generated_path and args.clean_rate != 0:
        ...
        train_generated_clean = torchvision.datasets.ImageFolder(
            os.path.join(args.clean_generated_path, 'train'), transform=tf
        )
        num_clean = int(args.clean_rate * len(train_clean)) 
        
        indices = np.random.choice(len(train_generated_clean), num_clean, replace=False)
        ...
        if num_poison != 0:
            ...
            string = f"\nPreset Poisoning Rate: {args.poisoning_rate} \nActual Poisoning Rate: {len(train_bd)/(len(train_clean)+len(train_generated_clean)):.4f}\n\n"
    ...
    if args.clean_generated_path:
        vtf = torchvision.transforms.Compose([
            torchvision.transforms.Resize(val_resize),
            torchvision.transforms.CenterCrop(args.input_size),
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize(IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)
        ...
        val_generated_clean = torchvision.datasets.ImageFolder(
            os.path.join(args.clean_generated_path, 'test'), vtf
        )
    
    if args.eval_on_real_data:
        val_real_clean = torchvision.datasets.ImageFolder(
            os.path.join('/vinserver_user/jason/diffusion_bd/physical_data_rotated/clean'), val_transforms
        )
    ...
    if args.clean_generated_path and args.clean_rate != 0:
        string = f"Train clean real dataset: {len(train_generated_clean)} \nValidation clean real dataset: {len(val_generated_clean)} \n\n"
        write_to_log(log_file, string)
    ...
    if args.real_bd_path:
        val_bd_real_loader = torch.utils.data.DataLoader(
            val_real_bd,
            batch_size=args.bs,
            num_workers=args.num_workers,
            pin_memory=True,
    ...
    for ep in range(args.epochs):
        ...
        if args.real_bd_path:
            ...
            clean_pbar = tqdm(enumerate(val_bd_real_loader), total=len(val_bd_real_loader))
            for i, (img, target) in clean_pbar:
                img = img.to(device, non_blocking=True)
                target = target.to(device, non_blocking=True)
                with torch.no_grad():
            ...
            real_bd_asr = accuracy(targets_real_bd, targets_real_bd, topk=(1,))
        ...
        if args.use_train_generated:
            if generated_clean_acc > max_generated_clean_acc \
                or (generated_clean_acc > max_generated_clean_acc-clean_acc_tol and  asr > max_poison_asr):
        ...
        if args.clean_generated_path and args.clean_rate != 0:
            print(f"Generated Clean Accuracy: {clean_acc:.4f}, ASR: {asr:.4f}, Real Clean Accuracy: {generated_clean_acc:.4f}")
            print(f"Max Generated Clean Acc: {max_clean_acc:.4f}, Max ASR: {max_poison_asr:.4f}, Max Real Clean Acc: {max_generated_clean_acc:.4f}")
            string = f"Epoch {ep+1:^3d} || Generated Clean Accuracy: {clean_acc:^7.4f}, ASR: {asr:^7.4f}, Real Clean Accuracy: {generated_clean_acc:^7.4f} || Max Generated Clean Acc: {max_clean_acc:^7.4f}, Max ASR: {max_poison_asr:^7.4f}, Max Real Clean Accuracy: {max_generated_clean_acc:^7.4f}\n"
            write_to_log(log_file, string)
        else:
